...
class TestHealthCheck:
    ...
    @patch("ollama_chatbot.api.flask_app.ollama.list")
...
class TestChatEndpoint:
    ...
    @patch("ollama_chatbot.api.flask_app.ollama.chat")
...
class TestGenerateEndpoint:
    ...
    @patch("ollama_chatbot.api.flask_app.ollama.generate")
...
class TestLogging:
    ...
    @patch("ollama_chatbot.api.flask_app.logger")
    @patch("ollama_chatbot.api.flask_app.ollama.chat")
...
class TestValidation:
    ...
    def test_chat_temperature_not_number(self, flask_client):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "Hello", "temperature": "hot"}),
            content_type="application/json",
        )
        assert response.status_code == 400
        data = response.get_json()
    ...
    def test_chat_temperature_out_of_range(self, flask_client):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "Hello", "temperature": 3.5}),
            content_type="application/json",
        )
        assert response.status_code == 400
        data = response.get_json()
    ...
    def test_chat_model_not_string(self, flask_client):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "Hello", "model": 123}),
            content_type="application/json",
        )
        assert response.status_code == 400
        data = response.get_json()
...
class TestErrorHandling:
    ...
    def test_value_error_handling(self, mock_chat, flask_client):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "test", "stream": False}),
            content_type="application/json",
        )
        assert response.status_code == 400
        data = response.get_json()
    ...
    def test_generate_temperature_type_validation(self, flask_client):
        ...
        response = flask_client.post(
            "/generate",
            data=json.dumps({"prompt": "test", "temperature": "not a number"}),
            content_type="application/json",
        )
        assert response.status_code == 400
...
class TestIntegration:
    ...
    def test_full_workflow(self, mock_chat, mock_list, flask_client, mock_ollama_list, mock_ollama_chat):
        ...
        # 3. Send chat message
        chat_response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "Hello", "stream": False}),
            content_type="application/json",
        )
...
class TestAdvancedScenarios:
    ...
    def test_very_long_message(self, mock_chat, flask_client, mock_ollama_chat):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": long_message, "stream": False}),
            content_type="application/json",
        )
    ...
    def test_special_characters_in_message(self, mock_chat, flask_client, mock_ollama_chat):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": special_msg, "stream": False}),
            content_type="application/json",
        )
        assert response.status_code in [200, 400]
    ...
    def test_unicode_characters(self, mock_chat, flask_client, mock_ollama_chat):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": unicode_msg, "stream": False}),
            content_type="application/json",
        )
        assert response.status_code in [200, 400]
...
class TestEdgeCases:
    ...
    def test_extremely_high_temperature(self, flask_client):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "test", "temperature": 5.0}),
            content_type="application/json",
        )
        assert response.status_code == 400
    ...
    def test_negative_temperature(self, flask_client):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "test", "temperature": -1.0}),
            content_type="application/json",
        )
        assert response.status_code == 400
...
class TestSecurityAndValidation:
    ...
    def test_path_traversal_in_model(self, mock_chat, flask_client, mock_ollama_chat):
        ...
        response = flask_client.post(
            "/chat",
            data=json.dumps({"message": "test", "model": path_traversal}),
            content_type="application/json",
        )
