#!/bin/bash

#############################################
# Ollama Chatbot - Shutdown Streamlit
# This script stops only the Streamlit application
#############################################

set -e  # Exit on error

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
NC='\033[0m' # No Color

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Shutdown Streamlit${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""

#############################################
# Stop Streamlit
#############################################
echo -e "${YELLOW}Stopping Streamlit...${NC}"

STREAMLIT_PIDS=$(pgrep -f "streamlit run.*app_streamlit.py" 2>/dev/null || true)

if [ -z "$STREAMLIT_PIDS" ]; then
    echo -e "${CYAN}ℹ Streamlit is not running${NC}"
    echo ""
    exit 0
fi

# Display found processes
echo -e "${CYAN}Found Streamlit process(es):${NC}"
echo "$STREAMLIT_PIDS" | while read -r pid; do
    echo -e "  PID: $pid"
done
echo ""

# Stop processes
echo "$STREAMLIT_PIDS" | while read -r pid; do
    kill "$pid" 2>/dev/null || true
done

# Wait for processes to stop
sleep 1

# Verify stopped
REMAINING=$(pgrep -f "streamlit run.*app_streamlit.py" 2>/dev/null || true)
if [ -z "$REMAINING" ]; then
    echo -e "${GREEN}✓ Streamlit stopped successfully${NC}"
else
    echo -e "${YELLOW}⚠ Some processes may still be running${NC}"
    echo -e "${YELLOW}  Trying force kill...${NC}"
    echo "$REMAINING" | while read -r pid; do
        kill -9 "$pid" 2>/dev/null || true
    done
    sleep 1
    echo -e "${GREEN}✓ Streamlit force stopped${NC}"
fi

echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${GREEN}  ✅ Streamlit stopped${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}To restart Streamlit:${NC}"
echo -e "  ./launch_streamlit.sh"
echo ""
